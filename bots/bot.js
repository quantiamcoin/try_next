...
class DatabaseManager {
    ...
    initializeDatabase() {
        this.db.serialize(() => {
            ...
            // Add table for tracking claimed messages
            this.db.run(`
            ...
            // Clear claimed messages
            this.db.run('DELETE FROM claimed_messages');
        });
    }
    ...
    async claimMessage(messageId, botId) {
        const query = `
            INSERT OR IGNORE INTO claimed_messages (message_id, bot_id, claimed_at)
            VALUES (?, ?, datetime('now'))
        `;
        return new Promise((resolve, reject) => {
            this.db.run(query, [messageId, botId], function(err) {
                if (err) reject(err);
                // this.changes will be 1 if inserted, 0 if already exists
...
// Bot Class
let messageReplied = new Set(); // Track message IDs that have been replied to
...
class QuantikBot {
    ...
    async sendRandomMessage() {
        ...
        if (this.instanceId <= 15) {
            if (this.instanceId <= 5) {
                const schedule = [0, 0.5, 0.8, 1.2, 2.0];
    ...
    async handleMessage(msg) {
        ...
        try {
            ...
            if (msg.from.is_bot) {
                console.log('here');
                // Skip messages based on random counter
                const skipCount = Math.floor(Math.random() * 5) + 1;
                if (this.skipCounter < skipCount) {
                    this.skipCounter++;
                    return;
                }
                this.skipCounter = 0;
                ...
                // 80% chance to reply
                if (Math.random() > 0.2) {
                    const { messageReplies } = require('./data');
                    
                    // Find the category and message
                    let category;
                    for (const [cat, messages] of Object.entries(messageReplies)) {
                        if (messages[text]) {
                            category = cat;
                            break;
                        }
                    ...
                    if (category && messageReplies[category][text]) {
                        const possibleReplies = messageReplies[category][text];
                        ...
                        this.lastMessageTime = Date.now();
                        await this.dbManager.insertMessage("bot_reply", response, `bot_${this.instanceId}`);
                    }
                }
                return;
            }
            ...
            const claimed = await this.dbManager.claimMessage(msg.message_id, `bot_${this.instanceId}`);
            if (!claimed) return;

            // Use mapped replies for human messages too
            const { messageReplies } = require('./data');
            for (const [category, messages] of Object.entries(messageReplies)) {
                if (messages[text]) {
                    const possibleReplies = messages[text];
                    response = possibleReplies[Math.floor(Math.random() * possibleReplies.length)];
                    break;
                }
            }

            // Fallback to common phrases if no mapped reply found
            if (!response && commonPhrases[text.toLowerCase()]) {
                response = commonPhrases[text.toLowerCase()];
            }
            ...
            if (response) {
                ...
                this.lastSentMessages.add(response);
                this.lastMessageTime = Date.now();
                await this.dbManager.insertMessage("human_reply", response, `bot_${this.instanceId}`);
            }
        } catch (error) {
